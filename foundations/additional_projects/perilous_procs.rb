...
def chain_map(num, procs)
  procs.inject(num) { |acc, prc| prc.call(acc) }
end
...
def proc_suffix(sentence, hash)
  ...
  sentence.split.each do |char|
    new_word = char
    hash.each do |proc, val|
      new_word += val if proc.call(char)
    end
    new_words << new_word
  end
  new_words.join(' ')
...
def proctition_platinum(arr, *procs)
  ...
  procs.each_with_index do |proc, i|
    new_arr = []
    arr.each do |char|
      if proc.call(char) && !hash.values[0].to_a.include?(char)
        new_arr << char
      end
    end
    hash[i + 1] = new_arr
...
# The hash contains procs as both keys and values. The method should return a 
# new sentence where each word of the input sentence is changed by a value proc 
# if the original word returns true when passed into the key proc. If an 
# original word returns true for multiple key procs, then the value proc 
# changes should be applied in the order that they appear in the hash.

def procipher(sentence, hash)
  words = sentence.split(' ')
  new_words = words.map do |word|
    new_word = word
    hash.each do |proc_check, proc_change|
      new_word = proc_change.call(new_word) if proc_check.call(word)
    end
    new_word
  end
  new_words.join(' ')
...
def picky_procipher(sentence, hash)
  ...
  new_words = words.map do |word|
    new_word = word
    hash.each do |proc_check, proc_change|
      if proc_check.call(word)
        new_word = proc_change.call(new_word)
        break
      end
    end
    new_word
